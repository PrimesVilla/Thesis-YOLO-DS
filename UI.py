...
class WasteDetection:
    ...
    def __init__(self, capture, confidence_threshold, max_age, offset, line):
        ...
        self.special_label = tk.Label(self.root, text=f'Special: {self.special_count}', font=("Helvetica", 16))
    ...
    def predict(self, img):
        ...
        source=img, conf=self.confidence_threshold, stream=True, device=0)
        return results
    ...
    def update_gui(self):
        ...
        self.special_label.config(text=f'Special: {self.special_count}')
    ...
    def video_loop(self):
        ...
        while True:
            ...
            if key == ord('w'):  # If 'w' key is pressed
                ...
                cv2.imwrite(filename, img_tk)  # Save the current frame
...
# Example usage:
detector = WasteDetection(capture=1, confidence_threshold=0.5, max_age=3, offset=10, line=320)
detector()
